  ...
  const drawer = (
    <List>
      {options.map((option, index)=>(
        <ListItem key={option.id} button selected={handleSelected(index)} onClick={()=>handleListClick(index)} classes={{
      ...
      <section className={classes.section}>
        ...
        <nav>
          <Hidden smUp implementation='css'>
            ...
            classes={{
              paperAnchorDockedLeft:classes.temporaryDrawer
            }}
            >
              {drawer}
            </Drawer>
...
function SectionItem({type='text', text, formItem}){
  ...
  return(
    <div className={classes.sectionItemWrapper}>
      ...
      <TextField variant='outlined' fullWidth value={formItem} type={type} inputProps={{
